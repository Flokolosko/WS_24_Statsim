...
Funfact: When I tried to set a seed before runif, I get a straight line of points, which may indicates that runif may be kind of cyclic. Therefore I did not set a seed before the runif functions
...
generate.points.butterfly <- function(n){
  ...
  points(df$x,df$y,col=ifelse(df$is.inside,"darkgreen","darkred"),pch=19,cex=.46)
  legend("topright",legend=c("Inside","Outside"), col = c("darkgreen","darkred"),pch=c(19,19))
  }
...
We can see, the more points we plot, the more random points we plot, the closer we get to the real figure. Hence, montecarlo approximation is very useful for complex figures, where Integrating is very costly or difficult. To get the area estimate, we simply need to calculate the area of the rectangle, in which we created the random points. Then, we just need to get the percentage of how many of those randomly generated points are in the area and then multiply with the area of the rectange. We get following table by doing so:
...
area <- 5.5*6
last.table <- data.frame(n=c(10^2,10^3,10^4,10^5))
last.table %>% mutate(percentage = get.inside.percentage(n)) %>% mutate(area=area*percentage)
```